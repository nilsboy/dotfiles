...
BEGIN {
    @attributes
        = qw(agent default_headers local_address max_redirect max_size proxy timeout SSL_options verify_SSL);
    no strict 'refs';
...
sub mirror {
    ...
    close $fh
        or
        Carp::croak(qq/Error: Could not close temporary file $tempfile: $!\n/);
    if ( $response->{success} ) {
...
sub www_form_urlencode {
    ...
    @params % 2 == 0
        or
        Carp::croak("form data reference must have an even number of terms\n");
